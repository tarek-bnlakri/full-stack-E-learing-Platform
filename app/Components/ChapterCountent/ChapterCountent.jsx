...
const getData=async(chapterName,session)=>{
  ...
  const res= await fetch(`https://full-stack-e-learing-platform-with-tarek.vercel.app/api/chapter/?chapterName=${chapterName || ''}&userEmail=${session.user.email || ''}`,{cache:"no-store"})
  if(!res.ok){
