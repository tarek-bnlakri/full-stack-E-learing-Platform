...
async function CoursesWrapper({catid}) {
  ...
  const getData=async(catid)=>{
    try {
          const res= await fetch(`https://full-stack-e-learing-platform-with-tarek.vercel.app/api/course?catid=${catid || ""}&userEmail=${session.user.email}`,{method:"GET",cache:"no-store"})
          // if(!res.ok){
